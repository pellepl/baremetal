...
int uart_hal_deinit(unsigned int hdl, uint16_t rx_pin, uint16_t tx_pin, uint16_t rts_pin, uint16_t cts_pin)
{
    ...
    if (rx_pin != BOARD_PIN_UNDEF)
        (void)gpio_config(rx_pin, GPIO_DIRECTION_ANALOG, GPIO_PULL_NONE);
    if (tx_pin != BOARD_PIN_UNDEF)
        (void)gpio_config(tx_pin, GPIO_DIRECTION_ANALOG, GPIO_PULL_NONE);
    if (rts_pin != BOARD_PIN_UNDEF)
        (void)gpio_config(rts_pin, GPIO_DIRECTION_ANALOG, GPIO_PULL_NONE);
    if (cts_pin != BOARD_PIN_UNDEF)
        (void)gpio_config(cts_pin, GPIO_DIRECTION_ANALOG, GPIO_PULL_NONE);
