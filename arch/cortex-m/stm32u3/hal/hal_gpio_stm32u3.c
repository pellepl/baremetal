...
static GPIO_TypeDef *port_for_pin(uint16_t pin) {
    uint32_t port_ix = pin >> 4;
    if (port_ix >= sizeof(ports) / sizeof(ports[0])) return 0;
    return ports[port_ix];
}
...
int gpio_hal_config(uint16_t pin, gpio_direction_t dir, gpio_pull_t pull) {
    ...
    LL_GPIO_InitTypeDef cfg = {
        .Pin = ll_pin,
        .Mode = ll_mode,
        .Speed = ll_mode == LL_GPIO_MODE_ALTERNATE ? LL_GPIO_SPEED_FREQ_HIGH : LL_GPIO_SPEED_FREQ_MEDIUM,
        .OutputType = ll_output,
        .Pull = pull == GPIO_PULL_NONE ? LL_GPIO_PULL_NO : (pull == GPIO_PULL_DOWN ? LL_GPIO_PULL_DOWN : LL_GPIO_PULL_UP)
    };
    (void)LL_GPIO_Init(port, &cfg);
    return 0;
}

void gpio_hal_stm32u3_af(uint16_t pin, uint8_t af);
void gpio_hal_stm32u3_af(uint16_t pin, uint8_t af) {
    GPIO_TypeDef *port = port_for_pin(pin);
    if (port == 0) return;
    uint32_t ll_pin = pins[pin & 0xf];
...
int gpio_hal_disconnect(uint16_t pin, gpio_pull_t pull) {
    return gpio_hal_config(pin, GPIO_DIRECTION_ANALOG, pull);
}
