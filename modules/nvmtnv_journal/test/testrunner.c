...
static struct {
	...
	test_t *_last_test;
	int test_count;
	void (*on_stop)(test_t *t);
	test_res_t *failed;
	test_res_t *failed_last;
	test_res_t *stopped;
	test_res_t *stopped_last;
	FILE *spec;
	char incl_filter[256];
	char excl_filter[256];
} test_main;

void test_init(void (*on_stop)(test_t *t))
{
...
static char check_spec(const char *name)
{
	if (test_main.spec) {
...
static char check_incl_filter(const char *name)
{
	if (strlen(test_main.incl_filter) == 0)
		return 1;
	return strstr(name, test_main.incl_filter) == 0 ? 0 : 2;
}

static char check_excl_filter(const char *name)
{
	if (strlen(test_main.excl_filter) == 0)
		return 1;
	return strstr(name, test_main.excl_filter) == 0 ? 1 : 0;
}

void _add_test(test_f f, const char *name, void (*setup)(test_t *t), void (*teardown)(test_t *t), int non_default)
{
	if (f == 0)
		return;
	if (!check_spec(name))
		return;
	if (check_incl_filter(name) <= non_default)
		return;
	if (!check_excl_filter(name))
		return;
	DBGT("adding test %s\n", name);
...
static void add_res(test_t *t, test_res_t **head, test_res_t **last)
{
	test_res_t *tr = malloc(sizeof(test_res_t));
	memset(tr, 0, sizeof(test_res_t));
...
static void dump_res(test_res_t **head)
{
	test_res_t *tr = (*head);
	while (tr) {
		test_res_t *next_tr = tr->_next;
		printf("  %s\n", tr->name);
		free(tr);
		tr = next_tr;
	}
...
int run_tests(int argc, char **args)
{
	...
	int failed = 0;
	int stopped = 0;
	test_t *cur_t = test_main.tests;
	int i = 1;
	while (cur_t) {
		cur_t->setup(cur_t);
		test_t *next_test = cur_t->_next;
