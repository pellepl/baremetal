...
static __attribute__((section(".nobss"))) struct
{
    uint32_t magic;
...
void sys_init(void)
{
    ...
    else if (nobss.unexpected_reset)
    {
        printf("Unexpected reset\n");
    }

    if (reset_reas & (1 << 26))
        nobss.reset_reason = RR_PIN;
    if (reset_reas & (1 << 27))
        nobss.reset_reason = RR_PORPDR;
    if (reset_reas & (1 << 28))
        nobss.reset_reason = RR_SW;
    if (reset_reas & (1 << 29))
        nobss.reset_reason = RR_IWDG;
    if (reset_reas & (1 << 30))
        nobss.reset_reason = RR_WWDG;
    if (reset_reas & (1 << 31))
        nobss.reset_reason = RR_LOW_POWER;
}

reset_reason_t sys_reset_reason(uint32_t *bootcnt, uint32_t *unexpected)
{
    if (bootcnt)
        *bootcnt = nobss.bootcnt;
    if (unexpected)
        *unexpected = nobss.unexpected_reset;
