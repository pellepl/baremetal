...
static int cli_rtc_date(int argc, const char **argv)
{
    ...
    else
    {
        ...
        rtc_get_date_time(&dt);
        if (argc >= 1)
            dt.date.year = atoi(argv[0]);
        if (argc >= 2)
            dt.date.month = atoi(argv[1]) - 1;
        if (argc >= 3)
            dt.date.month_day = atoi(argv[2]) - 1;
        if (argc >= 4)
            dt.time.hour = atoi(argv[3]);
        if (argc >= 5)
            dt.time.minute = atoi(argv[4]);
        if (argc >= 6)
            dt.time.second = atoi(argv[5]);
        if (argc >= 7)
            dt.time.millisecond = atoi(argv[6]);
