// adapted from NRF SDK dtm_uart.c
...
static int exit_command_ix;
static int exit_command_len;
 ...
 */
uint32_t dtm_start(const char *exit_command)
{
    ...
    m_iterations_next_byte_max = m_iteration[UART_BAUD_RATE_115200];
    exit_command_ix = 0;
    ...
    while (running)
    {
        ...
        int x;
        while ( (x = uart_prodtest_poll()) != -1 ) {
            rx_byte = (uint8_t)x;
    
            if (rx_byte == exit_command[exit_command_ix]) {
                exit_command_ix++;
                if (exit_command_ix >= exit_command_len) {
                    // received stop code
                    running = 0;
                    break;
                }
