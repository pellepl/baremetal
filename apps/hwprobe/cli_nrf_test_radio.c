...
CLI_FUNCTION(cli_radio_tx_carrier_unmodulated, "rf_tx_carr_unmod", "tx unmodulated carrier");

static int cli_radio_tx_carrier_modulated(int argc, const char **argv) {
    return radio_tx_carrier(argc, argv, 1);
}
CLI_FUNCTION(cli_radio_tx_carrier_modulated, "rf_tx_carr_mod", "tx modulated carrier");
...
CLI_FUNCTION(cli_radio_tx_pattern, "rf_tx_pattern", "[rnd|11110000|11001100] set tx pattern");
...
CLI_FUNCTION(cli_toggle_dc, "rf_dcdc", "[0|1] set dcdc");
...
CLI_FUNCTION(cli_radio_rx, "rf_rx", "start rx");
...
CLI_FUNCTION(cli_rx_dump, "rf_rx_dump", "dump rxed data");

static int cli_radio_stop(int argc, const char **argv) {
    radio_sweep_end();
    return 0;
}
CLI_FUNCTION(cli_radio_stop, "rf_stop", "stop radio");
...
CLI_FUNCTION(cli_radio_get_rssi, "rf_rssi", "return rssi");
...
CLI_FUNCTION(cli_radio_spectrum, "rf_spectrum", "[<channel-start>, <channel-end>(, <times>)] display rf spectrum");

static int cli_radio_init(int argc, const char **argv) {
    radio_init();
    return 0;
}
CLI_FUNCTION(cli_radio_init, "rf_init", "radio init");
