...
static void nvm_init(void)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        if (res >= 0)
            res = nvmtnv_fix();
        if (res >= 0)
            res = nvmtnv_read(SETTING_ID, buf, sizeof(buf));
        if (res >= 0)
        ...
        if (i == 0 && res < 0)
        {
            ...
            if (res >= 0)
                res = nvmtnv_mount(sector + sector_cnt - 5);
            if (res >= 0)
                res = nvmtnv_write(SETTING_ID, (const uint8_t *)_str(BUILD_INFO_TARGET_NAME), strlen(_str(BUILD_INFO_TARGET_NAME)));
            if (res < 0)
