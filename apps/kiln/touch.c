...
static uint32_t process_sample_data(const uint16_t *data, uint8_t count) {
    ...
    uint16_t max_d1 = 0; uint8_t max_d1_ix = 0;
    uint16_t max_d2 = 0; uint8_t max_d2_ix = 1;
    for (uint8_t i = 0; i < count; i++) {
        uint16_t d = data[i];
        uint16_t delta = d > avg ? (d - avg) : (avg - d);
        if (delta > max_d1) {
            max_d1 = delta; max_d1_ix = i;
        } else if (delta > max_d2) {
            max_d2 = delta; max_d2_ix = i;
        }
    }
    // take new average
...
void touch_process(void) {
    ...
    int drag = 0;
    int release = 0;
    if (_touch_glitch_filter) _touch_glitch_filter--;
    if (irq || gpio) {
        if (_touch_glitch_filter) return;
        touch_get(&_sys->last_raw_touch);
        if (irq) _sys->touch_pressed_irq_flag = 0;
        if (_prev_touch == 0) {
            press = 1;
            _prev_touch = 1;
            _drag_cnt = 0;
    ...
    if (_sys->needs_calibration) {
        ...
        if (release) _sys->needs_calibration++;
    } else {
        if (press || drag || release) {
            ...
            // filtering
            {
                ...
                _sys->spatial_filter.ix++; if (_sys->spatial_filter.ix >= TOUCH_SPATIAL_FILTER_LEN) _sys->spatial_filter.ix = 0;
                if (_sys->spatial_filter.vals < TOUCH_SPATIAL_FILTER_LEN+1) _sys->spatial_filter.vals++;
                // get avg
                ...
                if (_sys->spatial_filter.vals >= TOUCH_SPATIAL_FILTER_LEN) {
                    ...
                    int max_d1 = -1; uint8_t max_d1_ix = 0;
                    int max_d2 = -1; uint8_t max_d2_ix = 0;
            ...
            if (press) touch_event.type = EVENT_PRESS;
            else if (release) touch_event.type = EVENT_RELEASE;
            else touch_event.type = EVENT_DRAG;
            touch_event.x = gx;
            touch_event.y = gy;
