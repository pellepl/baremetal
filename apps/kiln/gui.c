...
{
    if (settings_get_val_for_id(SETTING_AUDIO_KEY_CLICK))
        gui_beep(40, 400);
}
...
static void view_main_setup(uint32_t instance, uint32_t index, int recall)
{
    ...
    title_stat1.colbg = COL_BG;
    title_stat1.colfg = COL_TITLE_BG;
    title_stat2.colbg = COL_BG;
    title_stat2.colfg = COL_TITLE_BG;
    ui_repaint(&title_stat1.comp);
    ui_repaint(&title_stat2.comp);
}
static void view_main_teardown(uint32_t instance, uint32_t index, int remember)
{
    ...
    title_stat1.colbg = COL_TITLE_BG;
    title_stat1.colfg = COL_TITLE_FG;
    title_stat2.colbg = COL_TITLE_BG;
    title_stat2.colfg = COL_TITLE_FG;
    ui_repaint(&title_stat1.comp);
    ui_repaint(&title_stat2.comp);
}
...
static void update_title_status(void)
{
    ...
    if (gui_status_temp > 500)
        temp = ICON_SMALL_TEMP_HOT3;
    else if (gui_status_temp > 250)
        temp = ICON_SMALL_TEMP_HOT2;
    else if (gui_status_temp > 50)
        temp = ICON_SMALL_TEMP_HOT1;
    ...
    if (gui_status_ctrl)
    {
        if (gui_status_power >= 100)
            power = ICON_SMALL_POW_5;
        else if (gui_status_power >= 75)
            power = ICON_SMALL_POW_4;
        else if (gui_status_power >= 50)
            power = ICON_SMALL_POW_3;
        else if (gui_status_power >= 25)
            power = ICON_SMALL_POW_2;
        else
            power = ICON_SMALL_POW_1;
    }
    char ctrl = ICON_SMALL_NONE;
    if (gui_status_ctrl)
    {
        ctrl = gui_status_ctrl == CONTROL_MANUAL ? ICON_SMALL_MANUAL : ICON_SMALL_PROGRAM;
    }
...
static void *gui_setting_val_cb(uint32_t ix)
{
    ...
    {
    case SETTING_UI_KILN_TEMP:
        return (void *)gui_kiln_temp;
    case SETTING_UI_KILN_POWER:
        return (void *)gui_kiln_power;
    default:
        return 0;
    }
}
    ...
    while ((ui_event = ui_pop_event()))
    {
        ...
        case EVENT_SECOND:
        {
                ...
                (int32_t)settings_get_val_for_id(SETTING_TEMP_GRAPH_UPDATE))
            {
                ...
                if (gui_graph_thermo_div)
                    repaint |= temp_graph_update(thermo);
