...
static void ctrl_kiln_timer_cb(timer_t *t) {
    ...
    if (max_temp) gui_set_attention(ATT_MAX_TEMP);
    ...
    if (!max_temp && !ctrl.panic && ctrl.state) {
        ...
        ctrl.kiln_cycle_ix++;
        if (ctrl.kiln_cycle_ix >= CTRL_CYCLES) ctrl.kiln_cycle_ix = 0;
    } else {
        // turn off relays
        ctrl_elements_off();
        if (settings_get_val_for_id(SETTING_KILN_SIMULATION)) {
...
void ctrl_manual_set_temp(float temp) {
    ...
    if (ctrl.state == CONTROL_MANUAL) { // TODO handle state transitions
        ctrl.pid.set = temp;
    }
}
