...
static const user_setting_t settings[] = {
    (user_setting_t){ .id = SETTING_KILN_MAX_TEMP,
        .ui_name = "Max temp", .unit = UNIT_DEGREE_C,
        .min_value = (void *)20, .max_value = (void *)1300, .def_value = (void *)250
    },
    (user_setting_t){ .id = SETTING_KILN_MAX_TIME,
        .ui_name = "Max time", .unit = UNIT_HOURS,
        .min_value = (void *)1, .max_value = (void *)48, .def_value = (void *)2
    },
    (user_setting_t){ .id = SETTING_KILN_MAX_TIME_FULL_PWR,
        .ui_name = "Full power max time", .unit = UNIT_MINUTES,
        .min_value = (void *)0, .max_value = (void *)2880, .def_value = (void *)120
    },
    (user_setting_t){ .id = SETTING_CTRL_MAX_TEMP,
        .ui_name = "Controller max temp", .unit = UNIT_DEGREE_C,
        .min_value = (void *)20, .max_value = (void *)80, .def_value = (void *)50
    },
    (user_setting_t){ .id = SETTING_KILN_MIN_SWITCH_TIME,
        .ui_name = "Min switch time", .unit = UNIT_MILLISECONDS,
        .min_value = (void *)100, .max_value = (void *)30000, .def_value = (void *)1000
    },

    (user_setting_t){ .id = SETTING_PID_K_P,
        .ui_name = "PID Kp", .unit = UNIT_PER_TEN_THOUSAND,
        .min_value = (void *)0, .max_value = (void *)1000000, .def_value = (void *)1000
    },
    (user_setting_t){ .id = SETTING_PID_K_I,
        .ui_name = "PID Ki", .unit = UNIT_PER_TEN_THOUSAND,
        .min_value = (void *)0, .max_value = (void *)1000000, .def_value = (void *)0
    },
    (user_setting_t){ .id = SETTING_PID_K_D,
        .ui_name = "PID Kd", .unit = UNIT_PER_TEN_THOUSAND,
        .min_value = (void *)0, .max_value = (void *)1000000, .def_value = (void *)0
    },
    (user_setting_t){ .id = SETTING_PID_EPSILON,
        .ui_name = "PID epsilon", .unit = UNIT_PER_TEN_THOUSAND,
        .min_value = (void *)0, .max_value = (void *)1000000, .def_value = (void *)100000
    },


    (user_setting_t){ .id = SETTING_AUDIO_KEY_CLICK,
        .ui_name = "Key click sound", .unit = UNIT_BOOLEAN,
        .def_value = (void *)1
    },
    (user_setting_t){ .id = SETTING_TEMP_GRAPH_UPDATE,
        .ui_name = "Temp graph update", .unit = UNIT_SECONDS,
        .min_value = (void *)1, .max_value = (void *)60, .def_value = (void *)1
    },
    (user_setting_t){ .id = SETTING_TEMP_GRAPH_LENGTH,
        .ui_name = "Temp graph length", .unit = UNIT_INTEGER,
        .min_value = (void *)100, .max_value = (void *)SETTINGS_TEMP_GRAPH_LEN_MAX, .def_value = (void *)SETTINGS_TEMP_GRAPH_LEN_MAX
    },
    (user_setting_t){ .id = SETTING_KILN_SIMULATION,
        .ui_name = "Kiln simulation", .unit = UNIT_BOOLEAN
    },
    (user_setting_t){ .id = SETTING_TOUCH_CALIB,
        .ui_name = "Touch calibration", .unit = UNIT_RESETTABLE
    },
    (user_setting_t){ .id = SETTING_FACTORY,
        .ui_name = "Factory settings", .unit = UNIT_RESETTABLE
    },
    (user_setting_t){ .id = SETTING_UI_RESET,
        .ui_name = "Processor", .unit = UNIT_RESETTABLE
    }
};
...
uint32_t settings_get_index_for_id(setting_t id) {
    if (id > _SETTINGS_VOLATILE) return (uint32_t)id;
    return ix_id[id];
}
...
int settings_set_val(uint32_t ix, void *val) {
    ...
    if (res >= 0) {
        ...
        if (ix < _SETTINGS_VOLATILE) values[ix] = val;
        if (setting_change_cb) setting_change_cb(s->id, val);
    }
...
const char *settings_unit_str(uint32_t ix) {
    const user_setting_t *s = settings_get(ix);
    switch (s->unit) {
        case UNIT_DEGREE_C: return "\x60\x43";
        case UNIT_MILLISECONDS: return "ms";
        case UNIT_SECONDS: return "sec";
        case UNIT_MINUTES: return "min";
        case UNIT_HOURS: return "h";
        case UNIT_PER_TEN_THOUSAND: return "/10000";
        default: return "";
    }
}
...
void settings_get_val_as_string(uint32_t ix, char *dst, uint32_t n, int unit) {
    ...
    const user_setting_t *s = settings_get(ix);
    switch (s->unit) {
        case UNIT_BOOLEAN:      snprintf(dst, n, "%s", (int)v ? "ON": "OFF"); break;
        case UNIT_DEGREE_C:     snprintf(dst, n, "%d%s", (int32_t)v, unit?settings_unit_str(ix):""); break;
        case UNIT_PER_TEN_THOUSAND: snprintf(dst, n, "%d%s", (int32_t)v, unit?settings_unit_str(ix):""); break;
        case UNIT_MILLISECONDS: snprintf(dst, n, "%d%s", (uint32_t)v, unit?settings_unit_str(ix):""); break;
        case UNIT_SECONDS:      snprintf(dst, n, "%d%s", (uint32_t)v, unit?settings_unit_str(ix):""); break;
        case UNIT_MINUTES:      snprintf(dst, n, "%d%s", (uint32_t)v, unit?settings_unit_str(ix):""); break;
        case UNIT_HOURS:        snprintf(dst, n, "%d%s", (uint32_t)v, unit?settings_unit_str(ix):""); break;
        case UNIT_STRING:       snprintf(dst, n, "%s", (const char *)v); break;
        case UNIT_RESETTABLE:   snprintf(dst, n, "RESET"); break;
        default:                snprintf(dst, n, "%d", (int32_t)v); break;
    }
}
...
void settings_init(void) {
    ...
    if (nvmtnv_read(SETTING_FACTORY, (uint8_t *)&fact_reset, sizeof(fact_reset)) == ERR_NVMTNV_NOENT) {
        ...
        for (uint32_t ix = 0; ix < settings_count(); ix++) {
            const user_setting_t *s = settings_get(ix);
            if (s->id != SETTING_TOUCH_CALIB) (void)nvmtnv_delete(s->id);
        }
        uint32_t v = 0;
    ...
    for (uint32_t ix = 0; ix < settings_count(); ix++) {
        ...
        if (res < 0) {
            printf("no setting found for ix %d, id %d:%s, take default\n",ix, s->id, s->ui_name);
...
static int cli_settings_set(int argc, const char **argv) {
    ...
    if (setting == 0) return ERR_CLI_EINVAL;
    values[ix] = (void *)val;
    if (setting_change_cb) setting_change_cb(setting->id, (void *)val);
