...
#define RGB32toRGB16(x) (((x) & 0xf80000) >> 8 | ((x) & 0xfc00) >> 5 | ((x) & 0xf8) >> 3)
...
enum {
    _MEM16B_START = 0,
    PIN_DB0 = _MEM16B_START, PIN_DB1, PIN_DB2, PIN_DB3,
    PIN_DB4, PIN_DB5, PIN_DB6, PIN_DB7,
    PIN_DB8, PIN_DB9, PIN_DB10, PIN_DB11,
    PIN_DB12, PIN_DB13, PIN_DB14, PIN_DB15,
    PIN_RW, PIN_RD, PIN_RS, PIN_CS, 
    _MEM16B_END = PIN_CS,
    PIN_RST
};
...
void disp_fill(int16_t x0, int16_t y0, int w, int h, uint32_t color) {
    ...
    if (y0 < cy0) {h += (cy0-y0); y0 = cy0;}
    if (x0+w >= cx1) w = cx1 - x0;
    if (y0+h >= cy1) h = cy1 - y0;
    uint32_t j = w * h;
    if (j == 0) return;
    uint16_t dst_col = RGB32toRGB16(color);
...
void disp_sleep(void) {
  ...
  _lgdp4532_reg_wr(POWERCTRL1,0x0002);
}

void disp_wakeup(void) {
  _lgdp4532_reg_wr(POWERCTRL1,0x1670);
...
void disp_draw_hline(int16_t x0, int16_t y0, int w, uint32_t col, uint8_t thickness) {
    ...
    y0 = y0 - adj_min;
    if (y1 < cy0 || y0 > cy1) return;
    if (y1 > cy1) y1 = cy1;
    if (y0 < cy0) y0 = cy0;
    uint16_t c = RGB32toRGB16(col);
    _lgdp4532_reg_wr(HORIZONTALRAMPOSITIONSTART,239-(y1));
    _lgdp4532_reg_wr(HORIZONTALRAMPOSITIONEND,239-(y0));
    _lgdp4532_reg_wr(HORIZONTALADDRESS,239-y0);
    _lgdp4532_reg_wr(VERTICALRAMPOSITIONSTART, x0);
...
void disp_draw_string(const char *str, int16_t x0, int16_t y0, 
    ...
    uint16_t fg16 = RGB32toRGB16(fg);
    uint16_t bg16 = RGB32toRGB16(bg);
    const font_t *font_hdl = font_get(font);
    if (font_hdl == 0) return;
    ...
    int16_t start_y = 0;
    if (y0 < cy0) start_y = cy0 - y0;
    if (y0 > cy1 || start_y > h) return;
    if (y0 + h > cy1) h = cy1 - y0;
