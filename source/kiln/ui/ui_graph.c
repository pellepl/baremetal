...
static void graph_paint(ui_component_t *ui, void *ctx, ui_info_t *i) {
    ...
    if (min > c->user_min) min = c->user_min;
    if (max < c->user_max) max = c->user_max;

    int ysep = 0;
    int y_grid = 0;
    {
        int32_t d = max-min;
        if (d >= 10000)     ysep = 2500;
        else if (d >= 5000) ysep = 1000;
        else if (d >= 2500) ysep = 500;
        else if (d >= 1000) ysep = 250;
        else if (d >= 500)  ysep = 100;
        else if (d >= 250)  ysep = 50;
        else if (d >= 100)  ysep = 25;
        else if (d >= 50)   ysep = 10;
        else if (d >= 25)   ysep = 5;
        else if (d >= 10)   ysep = 2;
        else if (d >= 5)    ysep = 1;
    }
    if (ysep) y_grid = (min / ysep) * ysep;
    ...
    {
        if (time_rep >= 3600*24)      xsep_s = 60*60*8;
        else if (time_rep >= 3600*12) xsep_s = 60*60*4;
        else if (time_rep >= 3600*8)  xsep_s = 60*60*2;
        else if (time_rep >= 3600*6)  xsep_s = 60*60;
        else if (time_rep >= 3600*3)  xsep_s = 30*60;
        else if (time_rep >= 3600*2)  xsep_s = 20*60;
        else if (time_rep >= 1200)    xsep_s = 10*60;
        else if (time_rep >= 900)     xsep_s = 5*60;
        else if (time_rep >= 600)     xsep_s = 2*60;
        else if (time_rep >= 300)     xsep_s = 1*60;
        else if (time_rep >= 120)     xsep_s = 30;
        else if (time_rep >= 60)      xsep_s = 20;
        else if (time_rep >= 30)      xsep_s = 10;
        else if (time_rep >= 10)      xsep_s = 5;
        else if (time_rep >= 5)       xsep_s = 1;
        xsep_s <<= ACC;
        xsep_g = xsep_s / (int32_t)settings_get_val_for_id(SETTING_TEMP_GRAPH_UPDATE);
    }
    int32_t starting_grid_sec = 0;
    if (xsep_s) {
        int32_t secs = 3600*c->_last_add_dt.time.hour 
            + 60*c->_last_add_dt.time.minute 
            + c->_last_add_dt.time.second;
        secs -= time_rep;
        while (secs < 0) secs += 3600*24;
        int32_t ss = secs % (xsep_s >> ACC);
        ss = (xsep_s >> ACC) - ss;
        starting_grid_sec = secs + ss;
        while (starting_grid_sec < 0) starting_grid_sec += 3600*24;
    ...
    for (uint16_t xx = x+1; xx < x + w; xx++) {
        ...
        if (x_grid) {
            next_ix_grid += xsep_g;
            if (first_x_grid < 0) first_x_grid = xx;
        }
...
void ui_graph_add(ui_graph_t *c, int16_t value) {
    ...
    c->_value_ix++;
    if (c->_value_ix >= c->_value_size) c->_value_ix = 0;
    if (new_limits) {
        c->_value_max = c->_value_min = value;
